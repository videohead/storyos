"""Workflow template system for StoryOS orchestrator.

Supports loading, validating, and rendering ComfyUI workflow templates
with placeholder substitution from Story Graph context data.
"""
...
class WorkflowTemplate:
    ...
    def __init__(self, data: dict[str, Any]):
        ...
        self.required_context_keys: list[str] = data.get("required_context_keys", [])
    ...
    def render(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        rendered_nodes = self._substitute_placeholders(rendered_nodes, context)
        ...
        return {
            ...
            "metadata": {
                ...
                "context_summary": {
                    k: context[k]
                    for k in self.required_context_keys[:5]  # limit for metadata
                },
            },
        }
    ...
    def _load_all(self):
        ...
        for path in self.templates_dir.glob("*.json"):
            try:
                ...
                print(f"Warning: Failed to load template {path.name}: {e}")
