<?php
/**
 * Tests for the StoryOS Secure Custom Fields schema archive.
 *
 * @package StoryOS
 */
 ...
 */
class Test_StoryOS_SCF_Alignment extends TestCase {
	...
	private function get_groups(): array {
		$groups = [];
		$files  = glob( dirname( __DIR__ ) . '/acf-json/group_storyos_*.json' );
	...
	public function test_every_cpt_has_an_archived_scf_group(): void {
		...
		foreach ( array_keys( $cpts ) as $cpt ) {
			$key = \StoryOS\Utils\SCF_Fields::group_key( $cpt );
	...
	public function test_archived_fields_match_the_cpt_manifest(): void {
		...
		foreach ( array_keys( \StoryOS\Utils\storyos_get_all_cpts() ) as $cpt ) {
			$group = $groups[ \StoryOS\Utils\SCF_Fields::group_key( $cpt ) ];
			$names = array_values( array_map( static function( array $field ): string {
				return (string) ( $field['name'] ?? '' );
			}, (array) $group['fields'] ) );

			$missing = array_diff( \StoryOS\Utils\storyos_expected_fields_for_cpt( $cpt ), $names );
	...
	public function test_archived_field_keys_are_stable_and_unique(): void {
		...
		foreach ( $this->get_groups() as $group ) {
			...
			$core_fields = \StoryOS\Utils\storyos_expected_fields_for_cpt( $cpt );
			foreach ( (array) $group['fields'] as $field ) {
				if ( in_array( (string) $field['name'], $core_fields, true ) ) {
					$this->assertSame(
						\StoryOS\Utils\SCF_Fields::field_key( $cpt, (string) $field['name'] ),
						$field['key']
					);
				}
	...
	public function test_archived_complex_fields_use_scf_storage_contracts(): void {
		$groups  = $this->get_groups();
		$project = array_column( $groups['group_storyos_project']['fields'], null, 'name' );
		$scene   = array_column( $groups['group_storyos_scene']['fields'], null, 'name' );
		...
		$this->assertSame( [ 'storyos_character' ], $project['team_members']['post_type'] );
