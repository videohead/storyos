...
use StoryOS\Utils\ElevenLabs_Catalog;
...
class Test_ElevenLabs extends TestCase {
	...
	public function test_endpoint_template_definitions(): void {
		...
		foreach ( $definitions as $definition ) {
			...
			$this->assertSame( 'audio', \StoryOS\Utils\Generation_Modality::output_type( $definition['modality'] ) );
		}
	}
	...
	public function test_audio_completion_contract(): void {
		...
		$this->assertLessThan(
			strpos( $batch, "update_post_meta( \$job_id, '_storyos_generation_result', \$result )" ),
			strpos( $batch, "unset( \$result['audio_data'] )" )
		);
