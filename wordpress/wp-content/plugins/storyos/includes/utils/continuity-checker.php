...
function auto_check_continuity_on_save( int $post_id, \WP_Post $post, bool $update = false ): void {
	...
	$validate_types = [ 'storyos_scene', 'storyos_shot' ];
	if ( ! in_array( $post->post_type, $validate_types, true ) ) {
		return;
	}

	// Check if validation was explicitly requested via a meta flag.
	$force = isset( $_POST['storyos_force_validation'] ) && wp_verify_nonce( $_POST['storyos_force_validation'] ?? '', 'storyos_validation' );
	if ( ! $force && ! $update ) {
	...
	$meta_key = STORYOS_CPT_PREFIX . 'continuity_issues';
	update_post_meta( $post_id, $meta_key, $result['issues'] );
	update_post_meta( $post_id, STORYOS_CPT_PREFIX . 'continuity_summary', [
...
function store_global_continuity_issues( array $issues ): void {
	$meta_key = STORYOS_CPT_PREFIX . 'global_continuity_issues';
...
function get_post_continuity_issues( int $post_id ): array {
	$issues = get_post_meta( $post_id, STORYOS_CPT_PREFIX . 'continuity_issues', true );
...
function get_post_continuity_summary( int $post_id ): array {
	$summary = get_post_meta( $post_id, STORYOS_CPT_PREFIX . 'continuity_summary', true );
...
function get_global_continuity_issues(): array {
	$issues = get_option( STORYOS_CPT_PREFIX . 'global_continuity_issues', [] );
	return is_array( $issues ) ? $issues : [];
}
...
function clear_post_continuity_issues( int $post_id ): bool {
	delete_post_meta( $post_id, STORYOS_CPT_PREFIX . 'continuity_issues' );
	delete_post_meta( $post_id, STORYOS_CPT_PREFIX . 'continuity_summary' );
	return true;
}
...
function clear_global_continuity_issues(): bool {
	delete_option( STORYOS_CPT_PREFIX . 'global_continuity_issues' );
	return true;
}
