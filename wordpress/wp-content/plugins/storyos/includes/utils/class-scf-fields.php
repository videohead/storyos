...
final class SCF_Fields {
	...
	private const ARCHIVE_HASH_OPTION = 'storyos_scf_archive_hash';

	/** Atomic lock for archive-to-database schema synchronization. */
	private const SCHEMA_SYNC_LOCK_OPTION = 'storyos_scf_schema_sync_lock';

	/** Durable diagnostics for a failed archive/database synchronization. */
	private const SCHEMA_SYNC_ERROR_OPTION = 'storyos_scf_schema_sync_error';
	...
	/** StoryOS group IDs awaiting one debounced Local JSON refresh. */
	...
	private static function maybe_sync_groups( array $definitions ): void {
		...
		$retry        = false;
		if ( ! $is_cli && isset( $_GET['storyos_scf_retry_schema'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- verified immediately below.
			check_admin_referer( 'storyos_scf_retry_schema' );
			$retry = true;
		}
	...
	private static function record_schema_sync_error( string $group_key, string $message ): void {
		...
		storyos_log( 'SCF schema synchronization error for ' . $group_key . ': ' . $message );
	}
	...
	public static function schema_sync_notice(): void {
		...
		$retry_url = wp_nonce_url(
			add_query_arg( 'storyos_scf_retry_schema', '1', admin_url( 'index.php' ) ),
			'storyos_scf_retry_schema'
		);
		echo '<div class="notice notice-error"><p><strong>'
			. esc_html__( 'StoryOS could not synchronize its SCF archive.', 'storyos' )
			. '</strong></p><ul>';
		...
		echo '</ul><p><a class="button" href="' . esc_url( $retry_url ) . '">'
			. esc_html__( 'Retry SCF synchronization', 'storyos' )
			. '</a></p></div>';
	}
	 ...
	 * then restored from StoryOS so an upgrade cannot break existing data. This
	 ...
	 * deletes every existing key that is absent. StoryOS imports instead use the
	...
	public static function protect_external_field_group_import( array $group ): array {
		...
		if ( is_wp_error( $db_fields ) || ! is_array( $db_fields ) ) {
			...
			wp_die(
				esc_html__( 'StoryOS blocked this SCF import because it could not safely preserve the existing custom fields.', 'storyos' ),
	...
	private static function persist_external_import_error( string $group_key, string $message ): void {
		...
		update_option(
			...
			[
				'version'      => self::SCHEMA_SYNC_VERSION,
				'archive_hash' => self::archive_hash( array_keys( storyos_get_all_field_defaults() ) ),
				'errors'       => $errors,
				'timestamp'    => time(),
			],
			false
		);
		storyos_log( 'SCF native import error for ' . $group_key . ': ' . $message );
	}
	...
	private static function merge_schema_fields( string $cpt, array $archive_fields, array $database_fields ) {
		...
		foreach ( $database_fields as $database_field ) {
			...
			if ( '' === $key ) {
				return new \WP_Error( 'storyos_scf_missing_field_key', __( 'An SCF field is missing its stable key.', 'storyos' ) );
			}
			...
			if ( isset( $archive_by_key[ $key ] ) ) {
				...
				if ( $canonical && (
					...
					return new \WP_Error(
						'storyos_scf_canonical_key_collision',
						sprintf( __( 'Database field key "%s" is already used by an incompatible field.', 'storyos' ), $key )
					);
				}
			...
			if ( '' === $name || isset( $seen_names[ $name ] ) ) {
				return new \WP_Error( 'storyos_scf_duplicate_field_name', sprintf( __( 'SCF sibling field name "%s" is empty or duplicated.', 'storyos' ), $name ) );
			}
		...
		foreach ( $archive_fields as $archive_field ) {
			...
			if ( '' === $key || '' === $name || isset( $seen_names[ $name ] ) ) {
				return new \WP_Error( 'storyos_scf_archive_field_collision', sprintf( __( 'Archived SCF field "%s" conflicts with a database-managed field.', 'storyos' ), $name ) );
			}
			$seen_names[ $name ] = true;
			$seen_keys[ $key ]   = true;
			$merged[]            = $field;
	...
	private static function merge_flexible_layouts( string $cpt, array $archive_layouts, array $database_layouts ) {
		...
		foreach ( $archive_layouts as $layout ) {
			...
			if ( '' === $key || isset( $archive_by_key[ $key ] ) ) {
				return new \WP_Error( 'storyos_scf_duplicate_layout_key', __( 'Flexible Content layout keys must be nonempty and unique.', 'storyos' ) );
			}
		...
		foreach ( $database_layouts as $database_layout ) {
			...
			if ( '' === $key || isset( $seen_keys[ $key ] ) ) {
				return new \WP_Error( 'storyos_scf_duplicate_database_layout_key', __( 'Database Flexible Content layout keys must be nonempty and unique.', 'storyos' ) );
			}
			...
			if ( '' === $name || isset( $seen_names[ $name ] ) ) {
				return new \WP_Error( 'storyos_scf_duplicate_layout_name', __( 'Flexible Content layout names must be nonempty and unique.', 'storyos' ) );
			}
			$seen_names[ $name ] = true;
			$seen_keys[ $key ]   = true;
			$merged[]            = $layout;
		...
		foreach ( $archive_layouts as $archive_layout ) {
			...
			if ( '' === $name || isset( $seen_names[ $name ] ) ) {
				return new \WP_Error( 'storyos_scf_archive_layout_collision', __( 'An archived Flexible Content layout conflicts with a database-managed layout.', 'storyos' ) );
			}
			$seen_names[ $name ] = true;
			$seen_keys[ $key ]   = true;
	...
	/** Keep each canonical group active and located only on its StoryOS CPT. */
	public static function enforce_canonical_group_contract( array $group ): array {
		...
		$group['show_in_rest'] = 'storyos_connection' === $cpt ? 0 : 1;
		return $group;
	}
	...
	private static function is_dynamic_choice_field( string $cpt, string $field_name ): bool {
		return in_array(
			$cpt . '.' . $field_name,
			[ 'storyos_connection.provider_type', 'storyos_template.modality', 'storyos_template.model_family' ],
			true
		);
	}
	...
	/** Queue an owning StoryOS group after standalone SCF field CRUD. */
	...
	/** Resolve a field parent chain and queue its canonical StoryOS group. */
	...
	/** Refresh each changed StoryOS group once after standalone field operations. */
	...
	private static function shape_raw_database_field( array $field ) {
		...
		if ( 'flexible_content' === $type ) {
			...
			if ( empty( $layouts ) ) {
				return new \WP_Error( 'storyos_scf_flexible_layout_missing', __( 'A Flexible Content field has children but no layouts.', 'storyos' ) );
			}
			...
			foreach ( $children as $child ) {
				...
				if ( ! array_key_exists( $layout_key, $layout_indexes ) ) {
					return new \WP_Error( 'storyos_scf_flexible_layout_orphan', __( 'A Flexible Content subfield references an unknown layout.', 'storyos' ) );
				}
		...
		return new \WP_Error(
			'storyos_scf_unsupported_nested_field',
			sprintf( __( 'Nested database fields under SCF type "%s" cannot be archived safely.', 'storyos' ), $type )
		);
	}
	...
	public static function maybe_migrate_legacy_values(): void {
		...
		if (
			...
			|| ! self::database_groups_have_canonical_fields( storyos_get_all_field_defaults() )
		) {
			return;
		}

		$retry_requested = false;
		if ( isset( $_GET['storyos_scf_retry_migration'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- verified immediately below.
			check_admin_referer( 'storyos_scf_retry_migration' );
	...
	private static function migrate_legacy_values(): void {
		...
		foreach ( $batch as $row ) {
			...
			if ( ! $post_id || ! isset( storyos_get_all_cpts()[ $cpt ] ) ) {
				continue;
			}
		...
		if ( ! self::persist_migration_state( $state ) ) {
			storyos_log( 'SCF legacy value migration could not persist its batch cursor.' );
		}
	}

	/**
	 * Convert all compatible legacy values on one StoryOS post.
	 *
	 * @return true|\WP_Error
	 */
	private static function migrate_legacy_post( int $post_id, string $cpt ) {
		if ( 'storyos_scene' === $cpt ) {
		...
		foreach ( storyos_get_field_defaults( $cpt ) as $field_name => $config ) {
			$field = self::get_field_object( $cpt, $field_name );
			if ( ! $field ) {
				return new \WP_Error( 'storyos_scf_canonical_field_missing', sprintf( __( 'Canonical SCF field %s is unavailable.', 'storyos' ), $field_name ) );
			}
			...
			if ( metadata_exists( 'post', $post_id, $field_name ) ) {
				...
				if ( (string) get_post_meta( $post_id, '_' . $field_name, true ) !== (string) $field['key'] ) {
					return new \WP_Error( 'storyos_scf_reference_migration_failed', sprintf( __( 'Could not backfill the SCF reference for %s.', 'storyos' ), $field_name ) );
				}
			}
			...
			if ( $normalized !== $raw_date ) {
				storyos_update_field_value( $post_id, $field_name, $normalized );
				if ( (string) get_post_meta( $post_id, $field_name, true ) !== $normalized ) {
					return new \WP_Error( 'storyos_scf_date_migration_failed', sprintf( __( 'Could not normalize the SCF date field %s.', 'storyos' ), $field_name ) );
				}
			}
		}
	...
	private static function normalize_legacy_date( string $value ) {
		...
		if ( preg_match( '/^(\d{4})(\d{2})(\d{2})$/', $value, $matches ) ) {
			return checkdate( (int) $matches[2], (int) $matches[3], (int) $matches[1] )
				? $value
				: new \WP_Error( 'storyos_scf_invalid_legacy_date', __( 'A legacy SCF date is not a real calendar date.', 'storyos' ) );
		}
		...
		return new \WP_Error( 'storyos_scf_unknown_legacy_date', __( 'A legacy SCF date uses an unsupported or invalid format.', 'storyos' ) );
	}
	...
	private static function migrate_legacy_dialogue( int $scene_id ) {
		$backup_key = '_storyos_scf_legacy_dialogue_backup';
		...
		if ( ! is_array( $raw ) ) {
			return $has_backup
				? new \WP_Error( 'storyos_scf_invalid_dialogue_backup', __( 'The preserved legacy dialogue backup is malformed.', 'storyos' ) )
				: true;
		}
		...
		foreach ( $raw as $index => $row ) {
			if ( ! is_array( $row ) ) {
				return new \WP_Error( 'storyos_scf_malformed_dialogue', sprintf( __( 'Dialogue row %d is malformed.', 'storyos' ), (int) $index + 1 ) );
			}

			$known_properties = [ 'speaker', 'line', 'description', 'sequence' ];
			$unknown          = array_diff( array_map( 'strval', array_keys( $row ) ), $known_properties );
			if ( ! empty( $unknown ) ) {
				return new \WP_Error( 'storyos_scf_unknown_dialogue_property', sprintf( __( 'Dialogue row %1$d contains unsupported data (%2$s).', 'storyos' ), (int) $index + 1, implode( ', ', $unknown ) ) );
			}

			foreach ( $known_properties as $property ) {
				if ( isset( $row[ $property ] ) && ! is_scalar( $row[ $property ] ) ) {
					return new \WP_Error( 'storyos_scf_malformed_dialogue_value', sprintf( __( 'Dialogue row %1$d has an invalid %2$s value.', 'storyos' ), (int) $index + 1, $property ) );
				}
			}
			if ( isset( $row['sequence'] ) && '' !== (string) $row['sequence'] && ! is_numeric( $row['sequence'] ) ) {
				return new \WP_Error( 'storyos_scf_malformed_dialogue_sequence', sprintf( __( 'Dialogue row %d has a nonnumeric sequence.', 'storyos' ), (int) $index + 1 ) );
			}
		...
		if ( ! $has_backup ) {
			...
			if ( $raw != get_post_meta( $scene_id, $backup_key, true ) ) {
				return new \WP_Error( 'storyos_scf_dialogue_backup_failed', __( 'StoryOS could not preserve the legacy dialogue before migration.', 'storyos' ) );
			}
		}
		...
		if ( is_array( $stored_count ) || (int) $stored_count !== count( $rows ) ) {
			return new \WP_Error( 'storyos_scf_dialogue_migration_failed', __( 'SCF did not persist the legacy dialogue as repeater rows.', 'storyos' ) );
		}

		foreach ( $rows as $index => $row ) {
			foreach ( $row as $name => $expected ) {
				if ( (string) get_post_meta( $scene_id, 'dialogue_' . $index . '_' . $name, true ) !== (string) $expected ) {
					return new \WP_Error( 'storyos_scf_dialogue_verification_failed', sprintf( __( 'SCF dialogue row %1$d did not preserve %2$s.', 'storyos' ), $index + 1, $name ) );
				}
			}
		}
	...
	private static function retry_legacy_migration_failures( array $state ): void {
		...
		if ( ! self::persist_migration_state( $state ) ) {
			storyos_log( 'SCF legacy value migration could not persist its failure state.' );
		}
	}
	...
	private static function complete_legacy_migration(): void {
		...
		if ( self::VALUE_MIGRATION_VERSION !== (int) get_option( self::VALUE_MIGRATION_OPTION, 0 ) ) {
			storyos_log( 'SCF legacy value migration could not record completion.' );
			return;
		}
	...
	public static function legacy_migration_notice(): void {
		...
		$retry_url = wp_nonce_url(
			add_query_arg( 'storyos_scf_retry_migration', '1', admin_url( 'index.php' ) ),
			'storyos_scf_retry_migration'
		);
		echo '<div class="notice notice-error"><p>'
			. esc_html( sprintf( __( 'StoryOS preserved %d legacy SCF value(s) that require review.', 'storyos' ), count( $state['failures'] ) ) )
			. ' <a href="' . esc_url( $retry_url ) . '">' . esc_html__( 'Retry migration', 'storyos' ) . '</a>'
			. '</p></div>';
	}
