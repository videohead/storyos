...
class ElevenLabs_Catalog {
	...
	const HOOK = 'storyos_provision_elevenlabs_templates';

	/** Register provisioning hooks only while this adapter is loaded. */
	public static function init(): void {
		add_action( self::HOOK, [ __CLASS__, 'provision' ] );
		add_action( 'save_post_storyos_connection', [ __CLASS__, 'schedule_after_connection_save' ], 20, 2 );
	}
	...
	public static function provision( int $connection_id ) {
		...
		if ( ! is_array( $connection ) || 'elevenlabs' !== ( $connection['provider_type'] ?? '' ) ) {
			return new WP_Error( 'elevenlabs_connection_invalid', __( 'Template provisioning requires an ElevenLabs Connection.', 'storyos' ) );
		}
		...
		if ( empty( $models ) || empty( $voices ) ) {
			$error = new WP_Error( 'elevenlabs_catalog_empty', __( 'ElevenLabs returned no usable text-to-speech models or voices.', 'storyos' ) );
		...
		if ( empty( $selected_voices ) ) {
			$error = new WP_Error( 'elevenlabs_voice_missing', __( 'None of the voice IDs allowed by this Connection are available.', 'storyos' ) );
			self::record_error( $connection_id, $error );
			return $error;
		}
	...
	private static function materialize( int $connection_id, array $definition ) {
		...
		if ( is_wp_error( $post_id ) || ! $post_id ) {
			return new WP_Error( 'elevenlabs_template_write_failed', __( 'StoryOS could not save the Template discovered from ElevenLabs.', 'storyos' ) );
		}
