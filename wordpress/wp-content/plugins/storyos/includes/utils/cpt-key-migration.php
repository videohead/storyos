<?php
/**
 * Compatibility migration for legacy StoryOS custom post type keys.
 *
 * @package StoryOS
 */
 ...
 */
function storyos_legacy_cpt_key_map(): array {
	return [
		'storyos_storyboard_frame'   => 'storyos_storyboard',
		'storyos_storyboard_f'       => 'storyos_storyboard',
		'storyos_editorial_artifact' => 'storyos_editorial',
		'storyos_editorial_ar'       => 'storyos_editorial',
	];
}
 ...
 */
function storyos_maybe_migrate_cpt_keys(): void {
	$target_version = 1;
	if ( (int) get_option( 'storyos_cpt_key_migration_version', 0 ) >= $target_version ) {
		return;
	}
	...
	$legacy_keys       = storyos_legacy_cpt_key_map();
	$migrated_post_ids = [];
	...
	if ( ! storyos_migrate_relationship_cpt_keys( $legacy_keys ) ) {
		return;
	}

	storyos_migrate_legacy_field_option_keys( $legacy_keys );
	update_option( 'storyos_cpt_key_migration_version', $target_version, false );
}
 ...
 */
function storyos_migrate_relationship_cpt_keys( array $legacy_keys ): bool {
	global $wpdb;
 ...
 * @param array<string, string> $legacy_keys Legacy-to-current CPT key map.
 */
function storyos_migrate_legacy_field_option_keys( array $legacy_keys ): void {
	...
	if ( $changed ) {
		update_option( 'storyos_fields', $all_fields, false );
	}
}
