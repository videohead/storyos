...
function fetch_graph_analytics( array $params = [] ) {
	...
	if ( ! $project_id || 'storyos_project' !== get_post_type( $project_id ) ) {
		return new \WP_Error( 'storyos_project_required', 'A valid StoryOS project is required for graph analysis.' );
	}
...
function fetch_character_network( array $params = [] ) {
	$project_id = absint( $params['project_id'] ?? 0 );
	if ( ! $project_id || 'storyos_project' !== get_post_type( $project_id ) ) {
		return new \WP_Error( 'storyos_project_required', 'A valid StoryOS project is required for character network analysis.' );
	}
...
function analyze_character_network( array $graph, int $limit = 0 ): array {
	...
	foreach ( $nodes as $node ) {
		...
		if ( 'storyos_character' === $node_types[ $id ] ) {
	...
	foreach ( $edges as $edge ) {
		...
		if ( $character_id && in_array( $entity_type, [ 'storyos_scene', 'storyos_shot' ], true ) ) {
			$bucket = 'storyos_scene' === $entity_type ? 'scenes' : 'shots';
			$presence[ $character_id ][ $bucket ][ $entity_id ] = true;
			if ( 'storyos_scene' === $entity_type ) {
	...
	return [
		'nodes'                    => array_values( array_filter( $nodes, static function( array $node ): bool { return 'storyos_character' === ( $node['type'] ?? '' ); } ) ),
...
function cache_graph_analytics( array $analytics, int $ttl = 3600, int $project_id = 0 ): bool {
	return set_transient( 'storyos_graph_analytics_' . $project_id, $analytics, $ttl );
}
...
function get_cached_graph_analytics( int $project_id = 0 ) {
	return get_transient( 'storyos_graph_analytics_' . $project_id );
}
...
function clear_cached_graph_analytics( int $project_id = 0 ): bool {
	return delete_transient( 'storyos_graph_analytics_' . $project_id );
}
...
function cache_character_network( array $network, int $ttl = 3600, int $project_id = 0 ): bool {
	return set_transient( 'storyos_character_network_' . $project_id, $network, $ttl );
}
...
function get_cached_character_network( int $project_id = 0 ) {
	return get_transient( 'storyos_character_network_' . $project_id );
}
...
function clear_cached_character_network( int $project_id = 0 ): bool {
	return delete_transient( 'storyos_character_network_' . $project_id );
}
