<?php
/**
 * MCP-driven fal model discovery and StoryOS Template provisioning.
 *
 * @package StoryOS
 */
...
class Fal_Catalog {
	...
	const HOOK = 'storyos_provision_fal_templates';
	...
	/**
	 * Discover allowed fal endpoints and create/update their StoryOS Templates.
	 *
	...
	public static function provision( int $connection_id ) {
		...
		if ( ! is_array( $connection ) || 'fal' !== ( $connection['provider_type'] ?? '' ) ) {
			return new WP_Error( 'fal_connection_invalid', __( 'Template provisioning requires a fal Connection.', 'storyos' ) );
		}
		...
		if ( empty( $endpoint_ids ) ) {
			$error = new WP_Error( 'fal_catalog_empty', __( 'fal MCP returned no text-to-image models to provision.', 'storyos' ) );
	...
	private static function materialize( int $connection_id, string $endpoint_id, array $model, array $schema ) {
		...
		if ( is_wp_error( $post_id ) || ! $post_id ) {
			return new WP_Error( 'fal_template_write_failed', __( 'StoryOS could not save the Template discovered from fal MCP.', 'storyos' ) );
		}
