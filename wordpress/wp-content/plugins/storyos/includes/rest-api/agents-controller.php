<?php
/**
 * Agents REST API Controller for StoryOS.
 *
...
class Agents_Controller extends Base_Controller {
	...
	public function register_routes() {
		...
		register_rest_route( 'storyos/v1', '/agents/(?P<id>\d+)', [
	...
	public static function agent_action( WP_REST_Request $request ) {
		...
		if ( ! $agent || $agent->post_type !== 'storyos_agent' ) {
			return new WP_Error( 'agent_not_found', 'Agent not found.', [ 'status' => 404 ] );
		}

		// Get agent configuration.
		$config = get_post_meta( $agent_id, '_storyos_agent_config', true );
		if ( ! $config ) {
	...
	private static function log_action( int $agent_id, string $action, array $params, array $result ): void {
		...
		$logs = get_post_meta( $agent_id, '_storyos_agent_logs', true ) ?: [];
		$logs[] = $log_entry;
		...
		update_post_meta( $agent_id, '_storyos_agent_logs', $logs );
	}
