 ...
 *
 * Handles synchronization between StoryOS elements and Web Stories.
 *
 ...
 *
 * Manages bidirectional sync between StoryOS and Web Stories.
 */
class Sync {
	...
	private function __construct() {
		// Register hooks.
		add_action( 'storyos_web_stories_sync_story', [ $this, 'sync_story' ], 10, 2 );
		add_action( 'storyos_web_stories_sync_scene', [ $this, 'sync_scene' ], 10, 2 );
		add_action( 'storyos_web_stories_sync_all', [ $this, 'sync_all' ], 10, 0 );
	}

	/**
	 * Get the Web Stories API client.
	 *
	 * @return \StoryOSWebStories\API\Client|null
	 */
	private function get_client(): ?\StoryOSWebStories\API\Client {
		if ( ! \StoryOSWebStories\API\Client::is_web_stories_active() ) {
			return null;
		}

		return new \StoryOSWebStories\API\Client();
	}
	...
	public static function is_sync_enabled(): bool {
		$enabled = get_option( 'storyos_web_stories_sync_enabled', false );
		return (bool) $enabled;
	}

	/**
	 * Get stored Web Stories mapping for a StoryOS post.
	 *
	 * @param int $post_id The StoryOS post ID.
	 * @return array
	 */
	public function get_mapping( int $post_id ): array {
		return get_post_meta( $post_id, '_storyos_web_stories_mapping', true ) ?: [];
	}

	/**
	 * Store Web Stories mapping for a StoryOS post.
	 *
	 * @param int   $post_id The StoryOS post ID.
	 * @param array $mapping The mapping data.
	 */
	public function set_mapping( int $post_id, array $mapping ): void {
		update_post_meta( $post_id, '_storyos_web_stories_mapping', $mapping );
	}

	/**
	 * Get the Web Story ID for a StoryOS post.
	 *
	 * @param int $post_id The StoryOS post ID.
	 * @return int|null
	 */
	...
	/**
	 * Get the StoryOS Scene ID for a Web Story.
	 *
	 * @param int $story_id The Web Story post ID.
	 * @return int|null
	 */
	public function get_scene_id( int $story_id ): ?int {
		$mapping = get_post_meta( $story_id, '_storyos_web_stories_mapping', true ) ?: [];
	 ...
	 *
	 * When a Web Story is created/updated, create or update a corresponding StoryOS Scene.
	 *
	 * @param int   $story_id   The Web Story post ID.
	...
	public function sync_story( int $story_id, array $story_data = [] ): array {
		...
		if ( $scene_id ) {
			...
			if ( $scene_id ) {
				...
				update_post_meta( $scene_id, '_storyos_web_stories_mapping', $result['mapping'] );
			} else {
				$result['success'] = false;
	 ...
	 *
	 * When a StoryOS Scene is created/updated, create or update a corresponding Web Story.
	 *
	 * @param int   $scene_id   The StoryOS Scene post ID.
	...
	/**
	 * Create a StoryOS Scene from a Web Story.
	 *
	 * @param int   $story_id The Web Story post ID.
	...
	private function create_scene_from_story( int $story_id, \WP_Post $post, array $pages ) {
		...
		update_post_meta( $scene_id, '_storyos_web_stories_mapping', $mapping );
		update_post_meta( $story_id, '_storyos_web_stories_mapping', $mapping );

		return $scene_id;
	}

	/**
	 * Update a StoryOS Scene from a Web Story.
	 *
	 * @param int   $scene_id The StoryOS Scene post ID.
	 * @param WP_Post $post   The Web Story post object.
	 * @param array   $pages  The pages from the Web Story.
	...
	/**
	 * Create a Web Story from a StoryOS Scene.
	 *
	 * @param int   $scene_id   The StoryOS Scene post ID.
	...
	private function create_story_from_scene( int $scene_id, array $scene_data ): ?int {
		...
		$result = \StoryOSWebStories\API\Client::create_story( $story_data );
	...
	/**
	 * Update a Web Story from a StoryOS Scene.
	 *
	 * @param int   $story_id   The Web Story post ID.
	 * @param int   $scene_id   The StoryOS Scene post ID.
	 * @param array $scene_data The scene data.
	...
	private function update_story_from_scene( int $story_id, int $scene_id, array $scene_data ): array {
		...
		$result = \StoryOSWebStories\API\Client::update_story( $story_id, [
	...
	/**
	 * Build Web Story pages from StoryOS Scene data.
	 *
	 * @param array $scene_data The scene data.
	 * @return array
	 */
	...
	public function sync_all(): array {
		...
		$stories = \StoryOSWebStories\API\Client::get_stories();
