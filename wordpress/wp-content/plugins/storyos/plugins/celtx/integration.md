...
StoryOS is an open-source production management platform for interactive storytelling, games, and transmedia projects. This document describes the integration between StoryOS and the **Celtx GEM Bi-Directional API** (v1.0.4), enabling bidirectional synchronization of production elements between the two platforms.

### Goals

- Synchronize StoryOS custom post types (CPTs) with Celtx elements (characters, locations, props, scenes, scripts).
- Maintain a persistent mapping between StoryOS and Celtx element IDs for bidirectional sync.
...
3. Copy the token and store it securely in StoryOS settings.
...
The integration maps StoryOS CPTs to Celtx elements as follows:

| StoryOS CPT | Celtx Target | Storage |
|-------------|-------------|---------|
| `storyos_project` | Celtx Project (Elements) | `_storyos_celtx_mapping` post meta |
| `storyos_character` | Celtx Element (`category=character`) | `_storyos_celtx_mapping` post meta |
| `storyos_location` | Celtx Element (`category=location`) | `_storyos_celtx_mapping` post meta |
| `storyos_scene` | Celtx Scene (episode/scene endpoints) | `_storyos_celtx_mapping` post meta |
| `storyos_shot` | Celtx Scene Comment (no direct shot concept) | `_storyos_celtx_mapping` post meta |
| `storyos_asset` / `storyos_prop` | Celtx Element (`category=prop`) | `_storyos_celtx_mapping` post meta |

### Mapping Data Structure

Each StoryOS post stores its Celtx mapping in post meta under the key `_storyos_celtx_mapping`:
...
**StoryOS fields → Celtx mapping:**

| StoryOS Meta Key | Celtx Field |
|------------------|-------------|
| `project_name` | `name` |
...
- Each StoryOS shot creates or updates a comment on the corresponding Celtx scene.
...
## 7. REST API Endpoints (StoryOS Side)

StoryOS exposes the following internal REST API endpoints for managing Celtx sync:

| Method | Endpoint | Description |
|--------|----------|-------------|
| `GET` | `/storyos/v1/celtx/test` | Test Celtx API connection |
| `POST` | `/storyos/v1/celtx/sync` | Sync all StoryOS elements |
| `GET` | `/storyos/v1/celtx/sync` | Get sync status |
| `POST` | `/storyos/v1/celtx/sync/{type}` | Sync all elements of a type |
| `POST` | `/storyos/v1/celtx/sync/{type}/{id}` | Sync an individual element |
| `GET` | `/storyos/v1/celtx/mapping/{type}/{id}` | Get Celtx mapping for an element |
| `DELETE` | `/storyos/v1/celtx/unsync/{type}/{id}` | Remove Celtx mapping |
...
```bash
curl -X GET "http://storyos.local/wp-json/storyos/v1/celtx/test" \
...
```bash
curl -X POST "http://storyos.local/wp-json/storyos/v1/celtx/sync" \
  -H "Authorization: Bearer YOUR_WP_JWT_TOKEN"
```
...
|----------|----------|--------|
| Missing credentials | 400 Bad Request | Configure API key in StoryOS settings |
...
3. **Webhooks / Callbacks:** Does Celtx support webhooks for real-time sync notifications when elements are modified in the Celtx app? This would enable bidirectional sync in the reverse direction (Celtx → StoryOS).
...
```
wordpress/wp-content/plugins/storyos/plugins/celtx/
...
For questions or support regarding this integration, please reach out to the StoryOS development team.
