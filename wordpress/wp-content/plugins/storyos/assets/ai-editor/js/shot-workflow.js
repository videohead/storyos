/* StoryOS filmmaking agent chat for classic Story Graph editors. */
( function() {
	'use strict';
	...
	function setBusy( workflow, isBusy, message ) {
		workflow.querySelectorAll( '.storyos-ai-workflow__run, .storyos-ai-workflow__agent, .storyos-ai-workflow__prompt, .storyos-ai-workflow__clear' ).forEach( function( element ) {
	...
	function appendMessage( workflow, role, content, label ) {
		var messages = workflow.querySelector( '.storyos-ai-workflow__messages' );
		var empty = messages.querySelector( '.storyos-ai-workflow__empty' );
		...
		message.className = 'storyos-ai-workflow__message storyos-ai-workflow__message--' + role;
		heading.textContent = label || ( 'user' === role ? 'You' : 'Agent' );
		body.className = 'storyos-ai-workflow__message-content';
	...
	function runWorkflow( workflow, action ) {
		...
		var history = getBoundedHistory( workflow.storyosMessages );
		var defaults = {
		...
		if ( ! prompt ) {
			workflow.querySelector( '.storyos-ai-workflow__status' ).textContent = 'Enter a message before sending.';
			return;
		}
		...
		workflow.storyosMessages.push( { role: 'user', content: prompt } );
		workflow.querySelector( '.storyos-ai-workflow__prompt' ).value = '';
		...
		request( '/ai/chat', { method: 'POST', body: JSON.stringify( body ) } ).then( function( response ) {
			...
			workflow.storyosMessages.push( { role: 'assistant', content: content } );
	...
	function clearChat( workflow ) {
		var messages = workflow.querySelector( '.storyos-ai-workflow__messages' );
		workflow.storyosMessages = [];
		messages.replaceChildren();
		var empty = document.createElement( 'p' );
		empty.className = 'storyos-ai-workflow__empty';
		empty.textContent = 'Start a conversation about this story element.';
		messages.append( empty );
		workflow.querySelector( '.storyos-ai-workflow__status' ).textContent = 'Chat cleared.';
	}
	...
	document.addEventListener( 'DOMContentLoaded', function() {
		document.querySelectorAll( '.storyos-ai-workflow' ).forEach( function( workflow ) {
			...
			workflow.querySelector( '.storyos-ai-workflow__prompt' ).addEventListener( 'keydown', function( event ) {
				if ( 'Enter' === event.key && ! event.shiftKey ) {
					event.preventDefault();
					runWorkflow( workflow, 'chat' );
				}
			} );
			workflow.querySelector( '.storyos-ai-workflow__clear' ).addEventListener( 'click', function() {
				clearChat( workflow );
			} );
		} );
	} );
