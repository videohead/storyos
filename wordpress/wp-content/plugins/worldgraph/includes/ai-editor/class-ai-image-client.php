 ...
 *
 * Talks to an OpenAI-compatible `/images/generations` endpoint so World Graph Studio can
...
class AI_Image_Client {
	...
	/**
	 * Image mime types World Graph Studio will accept back from a provider.
	 *
	 * @var array<int, string>
	 */
	...
	/**
	 * Largest generated image World Graph Studio will store, in bytes.
	 */
	...
	public function generate( string $prompt, array $options = [] ) {
		...
		if ( '' === $prompt ) {
			return new WP_Error( 'worldgraph_image_prompt_missing', __( 'A text-to-image prompt is required.', 'worldgraph' ), [ 'status' => 400 ] );
		}

		$base_url = $this->base_url();
		if ( '' === $base_url ) {
			return new WP_Error(
				'worldgraph_image_endpoint_missing',
				__( 'Set an image generation base URL in World Graph Studio AI Settings before generating assets.', 'worldgraph' ),
				[ 'status' => 400 ]
			);
		}
		...
		if ( is_wp_error( $response ) ) {
			return new WP_Error( 'worldgraph_image_unreachable', $response->get_error_message(), [ 'status' => 502 ] );
		}
		...
		if ( $status < 200 || $status >= 300 ) {
			if ( is_array( $body ) && isset( $body['error']['message'] ) ) {
				...
				$message = __( 'The configured image endpoint does not support the OpenAI-compatible /images/generations API. Configure an image provider that supports this API in World Graph Studio AI Settings.', 'worldgraph' );
			} else {
				$message = __( 'The image provider rejected the request.', 'worldgraph' );
			}

			return new WP_Error( 'worldgraph_image_request_failed', $message, [ 'status' => 502 ] );
		}

		if ( ! is_array( $body ) || empty( $body['data'][0] ) || ! is_array( $body['data'][0] ) ) {
			return new WP_Error( 'worldgraph_image_invalid_response', __( 'The image provider returned an unexpected response.', 'worldgraph' ), [ 'status' => 502 ] );
		}
	...
	private function extract_bytes( array $item ) {
		if ( ! empty( $item['b64_json'] ) && is_string( $item['b64_json'] ) ) {
			...
			if ( false === $decoded || '' === $decoded ) {
				return new WP_Error( 'worldgraph_image_decode_failed', __( 'The generated image could not be decoded.', 'worldgraph' ), [ 'status' => 502 ] );
			}
		...
		if ( empty( $item['url'] ) || ! is_string( $item['url'] ) ) {
			return new WP_Error( 'worldgraph_image_missing_payload', __( 'The image provider did not return image data.', 'worldgraph' ), [ 'status' => 502 ] );
		}

		// wp_safe_remote_get blocks requests to private/internal hosts.
		$download = wp_safe_remote_get( $item['url'], [ 'timeout' => 60 ] );
		if ( is_wp_error( $download ) ) {
			return new WP_Error( 'worldgraph_image_download_failed', $download->get_error_message(), [ 'status' => 502 ] );
		}

		$code = wp_remote_retrieve_response_code( $download );
		if ( $code < 200 || $code >= 300 ) {
			return new WP_Error( 'worldgraph_image_download_failed', __( 'The generated image could not be downloaded.', 'worldgraph' ), [ 'status' => 502 ] );
		}
	...
	private function validate_bytes( string $bytes ) {
		if ( '' === $bytes || strlen( $bytes ) > self::MAX_IMAGE_BYTES ) {
			return new WP_Error( 'worldgraph_image_invalid_payload', __( 'The generated image is empty or too large to store.', 'worldgraph' ), [ 'status' => 502 ] );
		}
		...
		if ( ! is_array( $info ) || empty( $info['mime'] ) || ! in_array( $info['mime'], self::ALLOWED_MIME_TYPES, true ) ) {
			return new WP_Error( 'worldgraph_image_unsupported_type', __( 'The provider returned a file that is not a supported image.', 'worldgraph' ), [ 'status' => 502 ] );
		}
	...
	private function base_url(): string {
		$url = trim( (string) get_option( 'worldgraph_ai_image_url', '' ) );
		if ( '' === $url ) {
	...
	private function api_key(): string {
		if ( defined( 'WORLDGRAPH_AI_IMAGE_API_KEY' ) && '' !== trim( (string) WORLDGRAPH_AI_IMAGE_API_KEY ) ) {
			return trim( (string) WORLDGRAPH_AI_IMAGE_API_KEY );
		}

		$key = trim( (string) get_option( 'worldgraph_ai_image_api_key', '' ) );

		return '' !== $key ? $key : trim( (string) get_option( 'worldgraph_ai_api_key', '' ) );
	}
	...
	private function model(): string {
		$model = trim( (string) get_option( 'worldgraph_ai_image_model', '' ) );
	...
	private function size( string $requested ): string {
		...
		$configured = trim( (string) get_option( 'worldgraph_ai_image_size', '' ) );
