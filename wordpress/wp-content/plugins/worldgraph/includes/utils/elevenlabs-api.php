...
class ElevenLabs_API {
	...
	public static function catalog( int $connection_id ) {
		...
		if ( ! is_array( $connection ) || 'elevenlabs' !== ( $connection['provider_type'] ?? '' ) ) {
			return new WP_Error( 'elevenlabs_connection_invalid', __( 'Select an ElevenLabs Connection first.', 'worldgraph' ) );
		}
	...
	public static function run_template( string $template_ref, string $prompt, array $parameters, int $connection_id = 0 ) {
		$connection = Connection_Repository::get( $connection_id );
		if ( ! is_array( $connection ) || 'elevenlabs' !== ( $connection['provider_type'] ?? '' ) ) {
			return new WP_Error( 'elevenlabs_connection_invalid', __( 'The selected Connection is not an ElevenLabs Connection.', 'worldgraph' ) );
		}

		$text = trim( wp_strip_all_tags( $prompt ) );
		if ( '' === $text ) {
			return new WP_Error( 'elevenlabs_text_missing', __( 'Enter a generation prompt.', 'worldgraph' ) );
		}
		...
		switch ( $method ) {
			case 'text_to_dialogue':
				...
				if ( '' === (string) ( $body['inputs'][0]['voice_id'] ?? '' ) ) {
					return new WP_Error( 'elevenlabs_voice_missing', __( 'The dialogue Template requires at least one voice ID.', 'worldgraph' ) );
				}
			...
			default:
				...
				if ( '' === $voice_id ) {
					return new WP_Error( 'elevenlabs_voice_missing', __( 'The text-to-speech Template requires a voice ID.', 'worldgraph' ) );
				}
	...
	private static function post_audio( array $connection, string $path, array $body, string $output_format, string $method ) {
		...
		if ( '' === $data ) {
			return new WP_Error( 'elevenlabs_audio_empty', __( 'ElevenLabs returned an empty audio response.', 'worldgraph' ) );
		}
	...
	private static function post_voice_design( array $connection, array $body, string $output_format ) {
		...
		if ( ! is_array( $decoded ) ) {
			return new WP_Error( 'elevenlabs_invalid_response', __( 'ElevenLabs Voice Design returned invalid JSON.', 'worldgraph' ) );
		}
		$items = [];
		...
		if ( empty( $items ) ) {
			return new WP_Error( 'elevenlabs_audio_empty', __( 'ElevenLabs Voice Design returned no audio previews.', 'worldgraph' ) );
		}
	...
	private static function json_get( string $url, array $headers ) {
		...
		return is_array( $decoded ) ? $decoded : new WP_Error( 'elevenlabs_invalid_response', __( 'ElevenLabs returned invalid JSON.', 'worldgraph' ) );
	}
	...
	private static function headers( string $credential_reference ) {
		...
		if ( '' === $key ) {
			return new WP_Error( 'elevenlabs_credential_missing', __( 'Set an ElevenLabs API key or env://ELEVENLABS_API_KEY reference on this Connection.', 'worldgraph' ) );
		}
