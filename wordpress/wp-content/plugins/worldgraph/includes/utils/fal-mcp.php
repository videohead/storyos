...
class Fal_MCP {
	...
	/** Tools World Graph Studio requires for asynchronous generation. */
	...
	public static function available_tools( int $connection_id ) {
		...
		if ( ! is_array( $connection ) || 'fal' !== ( $connection['provider_type'] ?? '' ) ) {
			return new WP_Error( 'fal_connection_invalid', __( 'Select a fal Connection first.', 'worldgraph' ) );
		}
	...
	public static function run_template( string $template, string $prompt, array $parameters, int $connection_id = 0 ) {
		...
		if ( '' === $endpoint_id ) {
			return new WP_Error( 'fal_endpoint_id_missing', __( 'The fal Template must specify a fal model endpoint ID.', 'worldgraph' ) );
		}
		$connection = Connection_Repository::get( $connection_id );
		if ( ! is_array( $connection ) || ! self::endpoint_is_allowed( $connection, $endpoint_id ) ) {
			return new WP_Error( 'fal_endpoint_not_allowed', __( 'That fal model endpoint is not allowed by the selected Connection.', 'worldgraph' ) );
		}
	...
	public static function get_job_status( string $job_id, int $connection_id = 0, string $endpoint_id = '' ) {
		if ( '' === trim( $endpoint_id ) ) {
			return new WP_Error( 'fal_endpoint_id_missing', __( 'The fal job is missing its model endpoint ID.', 'worldgraph' ) );
		}
	...
	private static function call_tool( string $name, array $arguments, int $connection_id ) {
		$connection = Connection_Repository::get( $connection_id );
		if ( ! is_array( $connection ) || 'fal' !== ( $connection['provider_type'] ?? '' ) ) {
			return new WP_Error( 'fal_connection_invalid', __( 'The selected Connection is not a fal Connection.', 'worldgraph' ) );
		}
		...
		$initialized = self::request_to( $endpoint, $credential, 'initialize', [
			'protocolVersion' => '2025-03-26',
			'capabilities'    => new \stdClass(),
			'clientInfo'      => [ 'name' => 'World Graph Studio WordPress', 'version' => defined( 'WORLDGRAPH_VERSION' ) ? WORLDGRAPH_VERSION : '1.0.0' ],
		] );
		if ( is_wp_error( $initialized ) ) {
			return $initialized;
		}
	...
	private static function request_to( string $endpoint, string $credential_reference, string $method, array $params, string $session_id = '' ) {
		...
		if ( '' === $api_key ) {
			return new WP_Error( 'fal_credential_missing', __( 'Set a fal API key or env://FAL_KEY reference on this Connection.', 'worldgraph' ) );
		}
		...
		if ( $status < 200 || $status >= 300 ) {
			$message = is_array( $payload ) ? (string) ( $payload['error']['message'] ?? __( 'fal MCP request failed.', 'worldgraph' ) ) : __( 'fal MCP request failed.', 'worldgraph' );
			return new WP_Error( 'fal_mcp_request_failed', $message, [ 'status' => $status ] );
		}
		if ( ! is_array( $payload ) ) {
			return new WP_Error( 'fal_mcp_invalid_response', __( 'fal MCP returned non-JSON content.', 'worldgraph' ) );
		}
		if ( isset( $payload['error'] ) ) {
			return new WP_Error( 'fal_mcp_error', (string) ( $payload['error']['message'] ?? __( 'fal MCP returned an error.', 'worldgraph' ) ), $payload['error'] );
		}
	...
	private static function decode_tool_result( array $result ) {
		if ( ! empty( $result['isError'] ) ) {
			$message = __( 'fal MCP tool call failed.', 'worldgraph' );
	...
	/** Map fal queue states onto World Graph Studio generation states. */
