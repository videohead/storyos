<?php
/**
 * Episodes REST API Controller for World Graph Studio.
 *
 * @package WorldGraph
 */
...
class Episodes_Controller extends Base_Controller {
	 ...
	 * @var string
	 */
	protected $cpt = 'worldgraph_episode';
	...
	public function register_routes() {
		register_rest_route( 'worldgraph/v1', '/episodes', [
			[
				'methods'             => 'GET',
		...
		register_rest_route( 'worldgraph/v1', '/episodes/(?P<id>\d+)', [
		...
		register_rest_route( 'worldgraph/v1', '/episodes/(?P<id>\d+)/graph', [
			'methods'             => 'GET',
	...
	public static function get_graph( \WP_REST_Request $request ) {
		...
		$entities = \WorldGraph\Utils\get_graph_entities( $post_id, 'worldgraph_episode' );
	...
	protected function get_item_data( \WP_Post $post, array $params = [] ): array {
		...
		$data['meta']['scene_count'] = self::count_related( $post->ID, 'worldgraph_scene', 'worldgraph_episode' );
		return $data;
	}
