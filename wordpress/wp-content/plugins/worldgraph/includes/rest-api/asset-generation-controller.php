<?php
/**
 * Asset Generation REST API Controller for World Graph Studio.
 *
...
use WorldGraph\Utils\Template_Bindings;
use WP_Error;
use WP_REST_Request;
...
class Asset_Generation_Controller extends Base_Controller {
	...
	public static function check_generate_permission( WP_REST_Request $request ) {
		...
		if ( ! current_user_can( 'upload_files' ) ) {
			return new WP_Error(
				'worldgraph_rest_forbidden_upload',
				__( 'You are not allowed to upload files to this site.', 'worldgraph' ),
				[ 'status' => 403 ]
			);
		}
	 ...
	 * or a World Graph Studio Details / SCF field; otherwise it's left out so a selection
