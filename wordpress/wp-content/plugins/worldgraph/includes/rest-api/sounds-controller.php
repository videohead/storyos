<?php
/**
 * Sounds REST API Controller for World Graph Studio.
 *
 * @package WorldGraph
 */
...
class Sounds_Controller extends Base_Controller {
	 ...
	 * @var string
	 */
	protected $cpt = 'worldgraph_sound';
	...
	public function get_items( $request ) {
		...
		$tax_query = [];
		foreach ( [ 'sound_type' => 'worldgraph_sound_type', 'production_status' => 'worldgraph_status' ] as $parameter => $taxonomy ) {
		...
		$posts    = array_values(
			array_filter(
				...
				function( \WP_Post $post ) use ( $scene_id, $shot_id ): bool {
					...
					if ( $scene_id && ! $this->has_relationship( $post->ID, $scene_id, 'worldgraph_scene' ) ) {
						return false;
					}

					if ( $shot_id && ! $this->has_relationship( $post->ID, $shot_id, 'worldgraph_shot' ) ) {
						return false;
					}
	...
	public function get_graph( \WP_REST_Request $request ) {
		...
		$entities = \WorldGraph\Utils\get_graph_entities( $post_id, $this->cpt );
	...
	protected function get_item_data( \WP_Post $post, array $params = [] ): array {
		...
		$types    = get_the_terms( $post->ID, 'worldgraph_sound_type' );
		$statuses = get_the_terms( $post->ID, 'worldgraph_status' );
	...
	/**
	 * Build links with public World Graph Studio route bases instead of CPT slugs.
	 *
	 * @param \WP_Post $post Sound post.
	 * @return array
	 */
	...
	private function validate_sound_request( \WP_REST_Request $request, bool $creating ) {
		...
		if ( $creating && ( ! is_scalar( $title ) || '' === trim( (string) $title ) ) ) {
			return new \WP_Error( 'worldgraph_sound_title_required', 'A Sound title is required.', [ 'status' => 400 ] );
		}
		...
		foreach ( [ 'spoken_text', 'lyrics', 'start_timecode', 'duration', 'diegetic', 'production_notes', 'scene', 'shot', 'character', 'asset' ] as $field ) {
			if ( array_key_exists( $field, $meta ) && null !== $meta[ $field ] && ! is_scalar( $meta[ $field ] ) ) {
				return new \WP_Error( 'worldgraph_sound_field_invalid', sprintf( 'The %s field must be a scalar value.', $field ), [ 'status' => 400 ] );
			}
		}

		if ( array_key_exists( 'diegetic', $meta ) && ! in_array( (string) $meta['diegetic'], [ '', 'unspecified', 'diegetic', 'non_diegetic', 'internal', 'mixed' ], true ) ) {
			return new \WP_Error( 'worldgraph_sound_diegetic_invalid', 'The diegetic value is invalid.', [ 'status' => 400 ] );
		}

		if ( ( $creating || array_key_exists( 'sound_type', $meta ) ) && empty( $meta['sound_type'] ) ) {
			return new \WP_Error( 'worldgraph_sound_type_required', 'A sound_type is required.', [ 'status' => 400 ] );
		}

		$sound_type_term = null;
		if ( array_key_exists( 'sound_type', $meta ) ) {
			if ( ! is_scalar( $meta['sound_type'] ) ) {
				return new \WP_Error( 'worldgraph_sound_type_invalid', 'A Sound has exactly one sound_type.', [ 'status' => 400 ] );
			}

			$term = is_numeric( $meta['sound_type'] )
				? get_term( absint( $meta['sound_type'] ), 'worldgraph_sound_type' )
				: get_term_by( 'slug', sanitize_title( (string) $meta['sound_type'] ), 'worldgraph_sound_type' );
			if ( ! $term || is_wp_error( $term ) ) {
				return new \WP_Error( 'worldgraph_sound_type_invalid', 'The sound_type must be an existing Sound Type term.', [ 'status' => 400 ] );
			}

			if ( \WorldGraph\Utils\worldgraph_is_reserved_sound_type( $term ) ) {
				return new \WP_Error( 'worldgraph_sound_type_reserved', 'Ordinary dialogue belongs in Scene dialogue metadata.', [ 'status' => 400 ] );
			}
			$sound_type_term = $term;
		} elseif ( ! $creating ) {
			$current_types = get_the_terms( absint( $request->get_param( 'id' ) ), 'worldgraph_sound_type' );
		...
		if ( array_key_exists( 'production_status', $meta ) ) {
			if ( ! is_scalar( $meta['production_status'] ) ) {
				return new \WP_Error( 'worldgraph_sound_status_invalid', 'A Sound has at most one production_status.', [ 'status' => 400 ] );
			}
			if ( '' !== (string) $meta['production_status'] ) {
				$status_term = is_numeric( $meta['production_status'] )
					? get_term( absint( $meta['production_status'] ), 'worldgraph_status' )
					: get_term_by( 'slug', sanitize_title( (string) $meta['production_status'] ), 'worldgraph_status' );
				if ( ! $status_term || is_wp_error( $status_term ) ) {
					return new \WP_Error( 'worldgraph_sound_status_invalid', 'The production_status must be an existing Status term.', [ 'status' => 400 ] );
				}
			}
		}
		...
		if ( ! $scene_id ) {
			return new \WP_Error( 'worldgraph_sound_scene_required', 'A Sound must belong to a Scene.', [ 'status' => 400 ] );
		}

		$targets = [
			'scene'     => 'worldgraph_scene',
			'shot'      => 'worldgraph_shot',
			'character' => 'worldgraph_character',
			'asset'     => 'worldgraph_asset',
		];
		foreach ( $targets as $field => $post_type ) {
			...
			if ( ! $target || $post_type !== $target->post_type ) {
				return new \WP_Error( 'worldgraph_sound_invalid_relationship', sprintf( 'The %s relationship is invalid.', $field ), [ 'status' => 400 ] );
			}

			if ( 'asset' === $field && ! \WorldGraph\Utils\worldgraph_is_audio_asset( $target->ID ) ) {
				return new \WP_Error( 'worldgraph_sound_asset_not_audio', 'The rendered Asset must have the Audio asset type.', [ 'status' => 400 ] );
			}
		}
		...
		if ( $shot_id && ! $this->shot_belongs_to_scene( $shot_id, $scene_id ) ) {
			return new \WP_Error( 'worldgraph_sound_shot_scene_mismatch', 'The selected Shot does not belong to the selected Scene.', [ 'status' => 400 ] );
		}
	...
	private function has_relationship( int $sound_id, int $target_id, string $target_cpt ): bool {
		foreach ( \WorldGraph\Utils\get_relationships( $sound_id, $this->cpt, 'outgoing' ) as $relationship ) {
			$field = 'worldgraph_scene' === $target_cpt ? 'scene' : 'shot';
			if (
