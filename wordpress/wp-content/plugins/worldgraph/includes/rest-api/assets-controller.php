<?php
/**
 * Assets REST API Controller for World Graph Studio.
 *
 * @package WorldGraph
 */
...
class Assets_Controller extends Base_Controller {
	 ...
	 * @var string
	 */
	protected $cpt = 'worldgraph_asset';
	...
	public function register_routes() {
		register_rest_route( 'worldgraph/v1', '/assets', [
			[
				'methods'             => 'GET',
		...
		register_rest_route( 'worldgraph/v1', '/assets/(?P<id>\d+)', [
		...
		register_rest_route( 'worldgraph/v1', '/assets/(?P<id>\d+)/graph', [
			'methods'             => 'GET',
	...
	public static function get_graph( \WP_REST_Request $request ) {
		...
		$entities = \WorldGraph\Utils\get_graph_entities( $post_id, 'worldgraph_asset' );
	...
	protected function get_item_data( \WP_Post $post, array $params = [] ): array {
		...
		$types = get_the_terms( $post->ID, 'worldgraph_asset_type' );
