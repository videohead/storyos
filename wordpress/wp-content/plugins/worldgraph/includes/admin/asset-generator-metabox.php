...
class Asset_Generator_MetaBox {
	...
	public static function enqueue( string $hook_suffix ): void {
		...
		wp_localize_script( 'worldgraph-asset-generator', 'worldgraphAssetGenerator', [
			...
			'i18n'    => [
				'generating'   => __( 'Generating image…', 'worldgraph' ),
				'queued'       => __( 'Image generation queued. WP-Cron will import the completed provider image into this post.', 'worldgraph' ),
				'job'          => __( 'Job', 'worldgraph' ),
				'loading'      => __( 'Building a prompt from this story element…', 'worldgraph' ),
				...
				'reloadHint'   => __( 'Reload the editor to see it in the featured asset and gallery fields.', 'worldgraph' ),
				'error'        => __( 'Image generation failed.', 'worldgraph' ),
				'unconfigured' => __( 'No text-to-image endpoint is configured. Set one in World Graph Studio AI Settings.', 'worldgraph' ),
			],
		] );
	}
	...
	private static function render_generator( \WP_Post $post ): void {
		?>
		<div class="worldgraph-generate-asset" data-post-id="<?php echo esc_attr( $post->ID ); ?>">
			<h4><?php esc_html_e( 'Generate asset', 'worldgraph' ); ?></h4>
			<p class="description"><?php esc_html_e( 'The result here is uploaded to the media library and linked to this post. You can ask the agents for specific assistance.', 'worldgraph' ); ?></p>
			<label for="worldgraph-generate-asset-prompt-<?php echo esc_attr( $post->ID ); ?>"><?php esc_html_e( 'Prompt', 'worldgraph' ); ?></label>
			<textarea class="widefat worldgraph-generate-asset__prompt" id="worldgraph-generate-asset-prompt-<?php echo esc_attr( $post->ID ); ?>" rows="4" placeholder="<?php esc_attr_e( 'Describe the image to generate.', 'worldgraph' ); ?>"></textarea>
			<p class="worldgraph-generate-asset__options">
				<label for="worldgraph-generate-asset-template-<?php echo esc_attr( $post->ID ); ?>"><?php esc_html_e( 'Template', 'worldgraph' ); ?></label>
