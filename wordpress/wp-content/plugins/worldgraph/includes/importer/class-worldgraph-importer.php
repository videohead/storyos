...
class WorldGraph_Importer {
	...
	private function validate_json( string $json ) {
		...
		// Sounds were added in World Graph Studio JSON 1.1. Treat the section as optional
	...
	private function import_project(): void {
		...
		\WorldGraph\Utils\worldgraph_update_field_value( $post_id, 'project_slug', \WorldGraph\Utils\sanitize_story_id( $external_id ) );
	...
	private function import_scenes(): void {
		...
		foreach ( $this->document['scenes'] as $scene ) {
			...
			// SCF fields.
			update_post_meta( $post_id, 'external_id', $external_id );
			\WorldGraph\Utils\worldgraph_update_field_value( $post_id, 'scene_number', $scene_index );
	...
	private function import_storyboards(): void {
		...
		foreach ( $this->document['storyboards'] as $frame ) {
			...
			// SCF fields.
			update_post_meta( $post_id, 'external_id', $external_id );
			\WorldGraph\Utils\worldgraph_update_field_value( $post_id, 'frame_number', $frame_index );
	...
	private function import_sequence(): void {
		...
		$term = term_exists( $sequence_title, 'worldgraph_sequence' );
		if ( ! $term ) {
			$term = wp_insert_term( $sequence_title, 'worldgraph_sequence' );
		}
		...
		\WorldGraph\Utils\worldgraph_set_sequence_order( (int) $term_id, 1 );
		...
		foreach ( $this->document['scenes'] as $scene ) {
			...
			if ( $this->overwrite && array_key_exists( 'characters', $scene ) && is_array( $scene['characters'] ) ) {
				...
				foreach ( $scene['characters'] as $char_external_id ) {
					$char_id = $this->id_map[ $char_external_id ] ?? 0;
					if ( $char_id ) {
						\WorldGraph\Utils\add_relationship( $scene_id, 'worldgraph_scene', $char_id, 'worldgraph_character', 'appears_in' );
					}
				}
			}
			...
			if ( $this->overwrite && array_key_exists( 'props', $scene ) && is_array( $scene['props'] ) ) {
				...
				foreach ( $scene['props'] as $prop_external_id ) {
					$prop_id = $this->id_map[ $prop_external_id ] ?? 0;
					if ( $prop_id ) {
						\WorldGraph\Utils\add_relationship( $scene_id, 'worldgraph_scene', $prop_id, 'worldgraph_prop', 'used_in' );
					}
				}
			}
		}
