...
( function( React, wp ) {
	...
	function AIEditorPanel() {
		...
		const [ currentPostId, setCurrentPostId ] = useState( window.worldgraphAI?.postId || 0 );

		// Load settings and agents on mount.
		useEffect( function() {
			fetch( window.worldgraphAI.restUrl + '/ai/settings?_wpnonce=' + window.worldgraphAI.nonce )
			...
			fetch( window.worldgraphAI.restUrl + '/ai/agents?_wpnonce=' + window.worldgraphAI.nonce )
				.then( function( response ) { return response.json(); } )
				.then( function( data ) {
					if ( data.success ) {
						setAgents( data.data );
		...
		function sendMessage() {
			...
			fetch( window.worldgraphAI.restUrl + '/ai/chat?_wpnonce=' + window.worldgraphAI.nonce, {
				method: 'POST',
				headers: {
	...
	// Register the panel.
	wp.registerBlockType( 'worldgraph/ai-editor-panel', {
		title: __( 'AI Editor' ),
		category: 'common',
		edit: function() {
	...
	if ( wp.editor && wp.editor.PluginSidebar ) {
		...
		wp.plugins.registerPlugin( 'worldgraph-ai-editor', {
			render: function() {
				return React.createElement( PluginSidebar, {
					name: 'worldgraph-ai-editor-sidebar',
					title: __( 'AI Editor' ),
					icon: 'admin-generic',
				},
	...
	document.addEventListener( 'keydown', function( event ) {
		...
		if ( ( event.ctrlKey || event.metaKey ) && event.key === 'Enter' ) {
			...
			var sidebar = document.querySelector( '.worldgraph-ai-editor-sidebar' );
			if ( sidebar && ! sidebar.classList.contains( 'is-collapsed' ) ) {
				// Dispatch a custom event that the React component can listen to.
				window.dispatchEvent( new CustomEvent( 'worldgraph-ai-send' ) );
			}
		}
	} );
