/**
 * World Graph Studio Plugins Manager JavaScript.
 *
 * @package WorldGraph
 */
...
(function($) {
	...
	$(document).ready(function() {
		 ...
		 */
		$(document).on('click', '.worldgraph-toggle-plugin', function(e) {
			e.preventDefault();
			...
			$.ajax({
				url: worldgraphPlugins.ajaxUrl,
				type: 'POST',
				data: {
					action: 'worldgraph_toggle_plugin',
					nonce: worldgraphPlugins.nonce,
					slug: pluginSlug
				},
				beforeSend: function() {
		 ...
		 */
		$(document).on('click', '.worldgraph-test-connection', function(e) {
			e.preventDefault();
