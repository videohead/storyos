 ...
 *
 * @package WorldGraphWebStories\REST
 */

namespace WorldGraphWebStories\REST;
...
class Sync_Controller {
	 ...
	 * @var string
	 */
	private $namespace = 'worldgraph-web-stories/v1';
	...
	public function register_routes(): void {
		...
		// Sync a specific World Graph Studio Scene.
		register_rest_route(
			$this->namespace,
		...
		// Get sync mapping for a World Graph Studio post.
		register_rest_route(
			$this->namespace,
			'/mapping/(?P<post_id>\d+)',
			[
				...
				[
					...
					'args'                => [
						...
						'sync_direction'     => [
							'required' => false,
							'type'     => 'string',
							'enum'     => [ 'bidirectional', 'worldgraph_to_web', 'web_to_worldgraph' ],
						],
						'auto_sync_on_save'  => [
							'required' => false,
							'type'     => 'boolean',
	...
	public function sync_story( \WP_REST_Request $request ) {
		...
		$result = \WorldGraphWebStories\Sync::init()->sync_story( $story_id );
	...
	public function sync_scene( \WP_REST_Request $request ) {
		...
		$result = \WorldGraphWebStories\Sync::init()->sync_scene( $scene_id );
	...
	public function sync_all( \WP_REST_Request $request ) {
		$result = \WorldGraphWebStories\Sync::init()->sync_all();
	...
	public function get_mapping( \WP_REST_Request $request ) {
		...
		$mapping = \WorldGraphWebStories\Sync::init()->get_mapping( $post_id );
	...
	public function get_status( \WP_REST_Request $request ) {
		$settings = \WorldGraphWebStories\Settings::init()->get_settings();
		...
		// Count mapped items.
		global $wpdb;
		$story_count = $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->postmeta} WHERE meta_key = '_worldgraph_web_stories_mapping' AND meta_value LIKE '%story_id%'" );
		$scene_count = $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->postmeta} WHERE meta_key = '_worldgraph_web_stories_mapping' AND meta_value LIKE '%scene_id%'" );
	...
	public function update_settings( \WP_REST_Request $request ) {
		...
		update_option( \WorldGraphWebStories\Settings::init()->option_name, $settings );
