...
class Settings {
	...
	public function render_settings_page(): void {
		...
		if ( 'POST' === ( $_SERVER['REQUEST_METHOD'] ?? '' ) && isset( $_POST['worldgraph_videodraft_nonce'] ) ) {
			...
			switch ( $action ) {
				...
				case 'list':
					...
					if ( is_wp_error( $result ) ) {
						...
						$message = sprintf( _n( '%d VideoDraft project found.', '%d VideoDraft projects found.', count( $result ), 'worldgraph' ), count( $result ) );
					}
					break;
				case 'push':
					$result = Sync::push( $project_id, $connection_id, $remote_project_id, isset( $_POST['force'] ) );
					$message = is_wp_error( $result ) ? $result->get_error_message() : sprintf( __( 'Exported to VideoDraft project %s.', 'worldgraph' ), $result['remote_project_id'] );
		...
		<div class="wrap">
			...
			<?php if ( empty( $connections ) ) : ?>
				<div class="notice notice-warning inline"><p><?php echo wp_kses_post( sprintf( __( 'Create a <strong>VideoDraft</strong> Connection before enabling sync. <a href="%s">Open Connections</a>.', 'worldgraph' ), esc_url( admin_url( 'edit.php?post_type=worldgraph_conn' ) ) ) ); ?></p></div>
			<?php endif; ?>
