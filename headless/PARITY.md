...
| Story Graph workspace | No headless browse, detail, create, or edit UI for the delivered Story Graph content types and SCF fields | Missing | Add a permission-aware headless contract and browse/edit workflows |
| Relationships, traversal, and ordering | No headless graph inspection, relationship editing, Scene/Shot ordering, or Sequence workflow | Missing | Match supported graph and ordering outcomes with cross-stack tests |
| Asset generation | No headless plan, generate, batch progress, cancel, history, or provenance UI | Missing | Consume the shared generation services and cover complete job states and permissions |
| Connections and Templates | `/connections` provides part of the ComfyUI catalog workflow through a server-side v1 adapter | Partial — production-blocked | Authenticate and authorize the browser user, define stable DTOs, preserve upstream errors, cover generic agreed workflows, and add contract/E2E tests |
| Story intelligence | No headless search, continuity, analytics, summary, or dramaturgy workflow | Missing | Deliver equivalent outcomes for shipped workflows or record explicit exceptions |
...
| Design system and content rendering | Design tokens are manually mirrored; post content uses rendered WordPress HTML | Partial | Establish canonical token/style ownership and test supported static, dynamic, and interactive block output |
| Cache invalidation | Shared-secret revalidation covers posts, pages, categories, and tags | Partial | Cover every headless-consumed entity, old/new slugs, graph aggregates, and failure/retry behavior |
...
| Cross-stack verification | No checked-in headless test suite or CI parity gate | Missing | Add contract, type/build, and browser tests including anonymous denial and authorized success |
