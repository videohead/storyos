...
| Cache invalidation | Story fetches use `story`, type, ID, and slug tags, and the shared-secret handler accepts the matching Story webhook shape; broader lifecycle and delivery behavior is not verified | Partial | Verify emits for publish/unpublish/delete and old/new slugs, invalidate related Scene/Project aggregates, and cover webhook failure/retry behavior |
