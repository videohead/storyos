# World Graph Studio Web Stories Sync Plugin

## Overview

The World Graph Studio Web Stories Sync plugin enables bidirectional synchronization between World Graph Studio elements (Scenes, Storyboard Frames) and Google Web Stories. Creators can export World Graph Studio scenes to interactive Web Stories for web publishing, or import completed Web Stories back into World Graph Studio for production tracking and asset management.

**Status**: 📋 Implemented — World Graph Studio Plugin (requires Web Stories plugin by Google)

## Supported Integrations

| Source | Target | Direction | Status |
|--------|--------|-----------|--------|
| World Graph Studio Scene | Web Story | Export | ✅ |
| Web Story | World Graph Studio Scene | Import | ✅ |
| World Graph Studio Storyboard Frame | Web Story Page | Export | ✅ |
| Web Story Page | World Graph Studio Scene | Import | ✅ |

## Features

### Export: World Graph Studio → Web Stories
- Convert World Graph Studio Scenes to Web Stories with one click
...
### Import: Web Stories → World Graph Studio
- Import Web Stories as World Graph Studio Scenes for production tracking
...
- **One-way sync** — World Graph Studio to Web Stories only, or Web Stories to World Graph Studio only
...
```
World Graph Studio (WordPress)                    Web Stories (WordPress)
┌─────────────────────┐               ┌─────────────────────┐
...
└─────────────────────┘               │  /sync/story/{id}   │
                                      │  /sync/scene/{id}   │
  Post Meta:                          │  /sync/all          │
  _worldgraph_web_stories_mapping        │  /mapping/{id}      │
  {                                  │  /status            │
...
1. **World Graph Studio** — installed and activated
...
1. The plugin is included with World Graph Studio at `wordpress/wp-content/plugins/worldgraph/plugins/web-stories/`
2. Activate both World Graph Studio and Web Stories plugins
3. Navigate to **World Graph Studio → Plugins → Web Stories Sync**
...
1. Navigate to **World Graph Studio → Scenes**
...
3. Click **Sync to World Graph Studio** (or save to trigger auto-sync)
4. A new World Graph Studio Scene is created with content from the story
5. The mapping is stored — future changes sync automatically

### Bulk Sync

1. Go to **World Graph Studio → Web Stories → Sync**
...
curl -X POST https://yoursite.com/wp-json/worldgraph-web-stories/v1/sync/scene/42 \
  -H "Authorization: Bearer YOUR_TOKEN"

# Sync a specific Web Story to Scene
curl -X POST https://yoursite.com/wp-json/worldgraph-web-stories/v1/sync/story/137 \
  -H "Authorization: Bearer YOUR_TOKEN"

# Sync all mapped items
curl -X POST https://yoursite.com/wp-json/worldgraph-web-stories/v1/sync/all \
  -H "Authorization: Bearer YOUR_TOKEN"

# Get sync status
curl https://yoursite.com/wp-json/worldgraph-web-stories/v1/status \
  -H "Authorization: Bearer YOUR_TOKEN"

# Get sync settings
curl https://yoursite.com/wp-json/worldgraph-web-stories/v1/settings \
  -H "Authorization: Bearer YOUR_TOKEN"

# Update sync settings
curl -X POST https://yoursite.com/wp-json/worldgraph-web-stories/v1/settings \
  -H "Authorization: Bearer YOUR_TOKEN" \
...
The plugin converts World Graph Studio scene content into Web Story pages based on the configured **Create Pages From** setting:
...
Web Story pages are converted to World Graph Studio scene content:
...
| World Graph Studio Field | Web Stories Equivalent |
|---------------|----------------------|
...
| `sync_direction` | `bidirectional` | Sync direction: `bidirectional`, `worldgraph_to_web`, `web_to_worldgraph` |
...
|--------|----------|-------------|
| `POST` | `/worldgraph-web-stories/v1/sync/story/{id}` | Sync Web Story → Scene |
| `POST` | `/worldgraph-web-stories/v1/sync/scene/{id}` | Sync Scene → Web Story |
| `POST` | `/worldgraph-web-stories/v1/sync/all` | Sync all mapped items |
| `GET` | `/worldgraph-web-stories/v1/mapping/{id}` | Get sync mapping for a post |
| `GET` | `/worldgraph-web-stories/v1/status` | Get sync status and counts |
| `GET` | `/worldgraph-web-stories/v1/settings` | Get sync settings |
| `POST` | `/worldgraph-web-stories/v1/settings` | Update sync settings |
...
```
wordpress/wp-content/plugins/worldgraph/plugins/web-stories/
...
- **Media assets** — Images and media from Web Story pages are not automatically imported into World Graph Studio media library
- **Storyboard Frames** — Optional feature, must be enabled in settings
- **Shots** — Shot CPT sync is planned for future releases
- **Web Stories plugin required** — Both World Graph Studio and Web Stories plugins must be active

## Future Enhancements

- [ ] Shot CPT sync support
- [ ] Media asset import/export
- [ ] Custom Web Story templates from World Graph Studio
- [ ] Batch import from Web Stories
- [ ] Web Story embedding in World Graph Studio Scene editor
...
For issues, feature requests, or contributions, please refer to the World Graph Studio documentation and governance guidelines.
